...
public class AmbientPlayEvent implements IACRCloudListener {

	private static final String TAG = "AmbientPlayEvent";
	...
	private OnAlarmListener mAmbientExpiredListener = new OnAlarmListener() {
		@Override
		public void onAlarm() {
			mIsEvent = false;
    ...
    public AmbientPlayEvent(Context context, EventsController controller) {
		mContext = context;
			...
			if (mResultCode == 0) {
				...
				if (metadata.has("music")) {
					...
					for (int t = 0; t < artists.length(); t++) {
						...
						mIsEvent = true;
						mController.notifyObservers();
						mAlarmManager.cancel(mAmbientExpiredListener);
						mAlarmManager.setExact(AlarmManager.RTC_WAKEUP, SystemClock.elapsedRealtime() 
						        + AMBIENT_EVENT_DURATION, "AmbientEvent", mAmbientExpiredListener, null);
					}
				}
				return;
			}
