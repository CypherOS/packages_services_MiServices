...
public class BatteryEstimateBridgeProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        long id = isMatched != -1 ? isMatched : db.insert(EstimatesDb.SQLITE_TABLE, null, values);
        return Uri.parse(ESTIMATE_URI + "/" + id);
    }

    private int isMatched(ContentValues values) {
        try (Cursor c = query(ESTIMATE_URI, PROJECTION, null, null, null)) {
            if (c != null) {
                if (c.moveToFirst()) {
                    return values.get(EstimatesDb.KEY_ESTIMATE).equals(c.getString(1)) ? c.getInt(0) : -1;
                }
            }
        }
        return -1;
